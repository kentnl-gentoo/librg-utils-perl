...
RG::Utils::Copf - Rost Lab perl utilities
...
See module source for description.

=over

=item amino_acid_convert_3_to_1

=item amino_acid_convert_3_to_1_ini
...
=item fctSeconds2time

=item gcgRd

=item gcgWrt

=item getDistanceNewCurveIde

=item getFileFormatQuick

=item get_in_keyboard

=item get_range

=item get_rangeHyphen

=item getSysARCH

=item hsspCorrectNali

=item hsspGetChain

=item hsspGetChainLength

=item hsspRdAli

=item hsspRdHeader

=item hsspRdSeqSecAccOneLine
...
=item rdRdbAssociative

=item rdRdbAssociativeNum
...
# 	    print $fh "*** ERROR $scrName after convPirmulGen\n","*** $msg\n" if (! $Lok || $Lok==2);}}
	...
	if ($formOut !~ /^(fasta|pir|gcg)/){
	    ...
	    Carp::confess("*** ERROR $scrName supported only PDB -> FASTA|PIR|GCG");}
	while (@fileIn) {
	    ++$itFile; $fileIn=shift @fileIn; $fileOutDef=shift @fileOutDef;
	    next if (! $par{"doAgain"} && -e $fileOutDef);
	    $chainIn=0;
	...
	if ($formOut !~ /^(fasta|pir|saf|msf|hssp)/){   # 
	    ...
	    Carp::confess("*** ERROR $scrName supported only DSSP -> FASTA|PIR|MSF|SAF2");}
	while (@fileIn) {
	    ++$itFile; $fileIn=shift @fileIn; $fileOutDef=shift @fileOutDef;
	    next if (! $par{"doAgain"} && -e $fileOutDef);
	    $chainIn=0;
	...
	while (@fileIn) {
		...
		Carp::confess("*** option PHD -> $formOut not implemented, yet!"); }
				# out: DSSP, nicer, HTML, ..
	    else {
		($Lok,$msg)=
	...
	Carp::confess("*** ERROR $scrName input format $formIn not supported");}
...
#	    next; }
#	$Lok= require $par{"$exe"};
# 	Carp::confess("*** $scrName: failed to require perl package '".$par{"$exe"}."'\n") if (! $Lok);}
    ...
    if ($par{"formatIn"} !~ /^($okFormInOr)$/ && $par{"formatIn"} !~ /^phdrdb/i )
    {
        my $msg = "*** ERROR $sbrName: input format '".$par{"formatIn"}."' (of files in $fileIn[1]) unsupported\n";
	cluck( $msg );
	return(0,$msg);
    }
	...
	$par{"fileOutScreen"}=$par{"dirWork"}.$par{"fileOutScreen"}
		 ...
		 $par{"fileOutScreen"} ne "unk" && length($par{"fileOutScreen"}) > 0);
                                ...
                                # trace file
                                # --------------------------------------------------
    if( $par{"fileOutTrace"} && $par{"fileOutTrace"} ne "unk" && 
...
# lkajan: there's got to be a better way to do this
#    if ($0=~/maxhom/){
...
# lkajan: there's got to be a better way to do this
#    if( $0=~/phd/ ){
#	$par{"dirSrcMat"}=      "/nfs/home1/yachdav/work/SNAP/phd/". "mat/";   # general material
#	$par{"dirPerl"}=        "/nfs/home1/yachdav/work/SNAP/phd/". "scr/";   # perl libraries
#	$par{"dirPerlScr"}=     "/nfs/home1/yachdav/work/SNAP/phd/". "scr/";   # perl scripts needed
#	$par{"dirBin"}=         "/nfs/home1/yachdav/work/SNAP/phd/". "bin/";   # FORTRAN binaries of programs needed
#    }
#    elsif( $0=~/prof/ ){
	$par{"dirSrcMat"}=      "$par{dirHome}/mat/";   # general material
...
#    }
    ...
    # lkajan: I see no reason to /set/ these by default
...
#    $par{"extOut"}=             ".tmp";
    ...
    $par{"verbose"}=$Lverb=     0; # blabla on screen
...
#    $par{"fileMatGcg"}=         $par{"dirSrcMat"}.    "Maxhom_Blosum.metric";  # MAXHOM-GCG matrix
				...
				# --------------------
				# executables
    $par{"exeConvertSeq"}=      "$par{dirConvertSeq}convert_seq";
    $par{"exeConvertSeqBig"}=   "$par{dirConvertSeq}convert_seq";
...
#-------------------------------------------------------------------------------
    ...
    # lkajan: not entirely sure - sourceFile is supposed to be module or command name?
    #if( ! $tmp ) { $tmp = __FILE__; }
...
#                    if (-e $par{$kwd} && ! -x $par{$kwd});
	    ...
	    $msgHere.="*** ERROR ".__FILE__.":".__LINE__." file ($kwd) '".$par{$kwd}."' missing!\n"
		if (! -e $par{$kwd} && ! -l $par{$kwd});} # 
    }
    if ($msgHere=~/ERROR/){ Carp::confess( $msgHere ); return(0,$msgHere); }
    return(1,"ok $sbrName");
}				# end of brIniErr
...
#	      " command line argument(s)",
	...
	if (defined $par{"fileHelpMan"} &&  -e $par{"fileHelpMan"}){
		...
		cluck "*** $sbrName: could NOT open file".$par{"fileHelpOpt"};
	    while(<FHIN>){
		print $_;}close(FHIN);}
	else {
	...
	if (defined $par{"fileHelpHints"} && -e $par{"fileHelpHints"}){
		...
		cluck "*** $sbrName: could NOT open file".$par{"fileHelpHints"};
	    while(<FHIN>){
	...
	if (defined $par{"fileHelpProblems"} && -e $par{"fileHelpProblems"}){
		...
		cluck "*** $sbrName: could NOT open file".$par{"fileHelpProblems"};
	    while(<FHIN>){
		print $_; ++$ct;}close(FHIN);}
	...
	$tmpSpecial=$tmp{"$tmp2"} if (! defined $tmp{"$tmp"} && defined $tmp{"$tmp2"});
		...
		&brIniHelpRdItself($tmp{"sourceFile"});
            Carp::confess('.....   verrry sorry the option blew up ... ') if (! $Lok);
	    $def{"kwd"}="" if (! $Lok); # hack: short cut error
            @kwdLoc=split(/,/,$def{"kwd"});$#tmp=0;
            foreach $kwd (@kwdLoc){
...
#-------------------------------------------------------------------------------
    ...
    if( !$par{dirWork} ){ $par{dirWork} = File::Temp::tempdir( CLEANUP => !$par{debug} ); }
	...
	foreach $kwd (@kwd){
	    next if ($kwd !~ /^file/);
	    next if ($kwd =~ /^file(In|Out|Help|Def|Mat)/i); # lkajan: infernal. This should be a next UNLESS /something/ condition.
            $par{$kwd}=~s/jobid/$par{"jobid"}/ ;
...
#-------------------------------------------------------------------------------
	...
	foreach $it(@extr){
	    $take{$it}=1;}}
				# open file
    open($fhinLoc,$fileInLoc) || Carp::confess( "failed to open $fileInLoc: $!" ) ||
	...
	$tmp{"NROWS"}=$fasta{"NROWS"};
        ...
        open("$fhout",">$fileOutLoc") || Carp::confess( "failed to open $fileOutLoc: $!" )  || # open file
...
#--------------------------------------------------------------------------------
    ...
    $dirDsspLoc=""              if (! defined $dirDsspLoc || ! -d $dirDsspLoc);
				...
				# ------------------------------
    if ($exeConv=~/\.pl/){
	system("$exeConv $fileFssp $dirDsspLoc >> $fileDafTmp") && Carp::confess( "cmd $exeConv $fileFssp $dirDsspLoc >> $fileDafTmp failed: ".( $? >> 8 ) ); }
				# ------------------------------
				# include as package
				# ------------------------------
    else {
...
#-------------------------------------------------------------------------------
	...
	Carp::confess("*** output format is HTML??? not yet in it!");
...
#----------------------------------------------------------------------
    ...
    open($fhin,$fileIn) || Carp::confess( "failed to open $fileIn: $!" ) ||
...
#----------------------------------------------------------------------
	...
	$Lread=1;
	if (length($_)<12){
	    Carp::confess("*** fatal ERROR in $fileIn: line=$ctline:$_!");}
	$chainRd=substr($_,12,1); 
	$pos=    substr($_,7,5); $pos=~s/\s//g;
...
# 01/10/10 GY
# WE ARE NO LONGER LOOKING FOR PLATFORM INFORMATION USING THIS METHOD
...
#    return(&errSbr("not def !"))          if (! defined $);
    ...
    return(&errSbr("no fileIn=$fileInLoc!"))  if (! -e $fileInLoc);
				...
				# open file
    open($fhinLoc,$fileInLoc) || Carp::confess( "failed to open $fileInLoc: $!" ) || return(&errSbr("fileInLoc=$fileInLoc, not opened"));
				# ------------------------------
	...
	"--- $sbrName system '$cmd'\n"
	    if ($par{"debug"});
				# now change
    open($fhinLoc,$fileTmpLoc) || Carp::confess( "failed to open $fileTmpLoc: $!" ) || return(&errSbr("in fileTmpLoc=$fileTmpLoc, not opened"));
    open($fhoutLoc,">".$fileInLoc) || Carp::confess( "failed to open $fileInLoc: $!" ) || return(&errSbr("out fileInLoc=$fileInLoc, not created"));
				# ------------------------------
    while (<$fhinLoc>) {	# read file
	if    ($_=~/^NALIGN \s+(\d+)/){
				# '....,....1....,'
...
#--------------------------------------------------------------------------------
    $fhinLoc2="FHIN_FASTA";
    open($fhinLoc2,$fileLoc) || Carp::confess( "failed to open $fileLoc: $!" );
    $one=(<$fhinLoc2>);
    $two=(<$fhinLoc2>);
    $two=~s/\s|\n//g            if (defined $two);
    close($fhinLoc2);
...
#--------------------------------------------------------------------------------
    ...
    $fhinLoc="FHIN_GCG";
    open($fhinLoc,$fileLoc) || Carp::confess( "failed to open $fileLoc: $!" ) || do { cluck "-*- isGcg failed opening=$fileLoc\n";
				    return(0); };
    $ctLocFlag=$#tmp=0;
...
#--------------------------------------------------------------------------------
    ...
    open($fhinLoc,$fileLoc) || Carp::confess( "failed to open $fileLoc: $!" ) || 
	do { cluck "*** isPdb failed opening file=$fileLoc!\n";
	     return(0); };
    $tmp="";
    while(<$fhinLoc>){
	$tmp=$_; $tmp=~s/\n//g;
...
#--------------------------------------------------------------------------------
    ...
    $lenok=1;			# length in header and of sequence differ
                                ...
                                # read the file
                                # ----------------------------------------
    open ($fhLoc,$fileMsf) || Carp::confess( "failed to open $fileMsf: $!" )  || 
...
#-------------------------------------------------------------------------------
	...
	$_=~s/\n//g;
	push(@tmp,$_);
        last if ($_ eq "//" ); # skip everything before ali sections
    } 
    undef %tmp;$ct=0;
...
#-------------------------------------------------------------------------------
    ...
    open("$fhinLoc","<", $fileInLoc) || do {
	Carp::confess( "failed to open <$fileInLoc: $!" );
	return(0,"*** ERROR $sbrName: fileIn=$fileInLoc, not opened\n");
    };
	...
	chomp;
	#Name: tsnap                Len: 332   Check: 2222 Weight: 1.00
        if($_=~ /^\s*NAME:\s*(\S+).*LEN:.*\d+\s*CHECK:\s*\d+\s*WEIGHT:/i){ ++$ct; }
        last if ($_ eq "//" );
        next;} close($fhinLoc);
    return($ct);
}				# end of msfCountNali
...
#-------------------------------------------------------------------------------
	...
	open ($file_handle, ">$temp_name") || Carp::confess( "failed to open $temp_name: $!" ) || ( do {
	    cluck "***\t Cannot create new file: $temp_name\n" ;
	    if ( $log_file ) {
	...
	cluck "*** ERROR lib-br:open_file: Cannot open file '$file_name'\n" ;
	if ( $log_file ) {
...
#SEQRES   1 A  198  ALA ASN ILE VAL GLY GLY ILE GLU TYR SER ILE ASN ASN  1P06  74
	...
	foreach $tmp (@tmp) {
	    next if ($tmp!~/^[A-Z]$/); # exclude strange stuff
	    ++$ctStrange;}
    } close($fhinLoc);
...
#-------------------------------------------------------------------------------
    ...
    $Lok=       open("$fhinLoc","$fileInLoc") || Carp::confess( "failed to open $fileInLoc: $!" );
...
#-------------------------------------------------------------------------------
    $sbrName="lib-sys:".( $par{"debug"} ? __FILE__.":".__LINE__ : '' ).":sysRunProg";
...
#    print "$cmd\n";exit;
    ...
    # cluck/longmess fails with some perl bug!!!
    if( $par{"debug"} ) { cluck( "|$prog" ); }
    open (CMD, "|$prog") || 
	Carp::confess( "*** $sbrName cannot open program '|$prog': $!" );
    ...
    if( !close(CMD) ){ confess( "cmd '|$prog' failed: ".( $? >> 8 ) ); } 	# upon closing: cmdtmp < @out_command executed    
    return(1,"ok $sbrName");
}				# end of sysRunProg
...
#-------------------------------------------------------------------------------
	...
	if ($fileIncl && -e $fileIncl){
	    open("$fhinLoc","$fileIncl") || confess( "failed to open $fileIncl: $!" ) ||
...
#-------------------------------------------------------------------------------
    ...
    $tmpWrt.=$tmp;
    if( $par{debug} ){ print $fhLoc "$tmpWrt\n"; }
}				# end of wrtLoc
