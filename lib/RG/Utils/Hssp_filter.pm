...
RG::Utils::Hssp_filter - Rost Lab perl utilities
...
=item hsspFilterGetCurveIde

=item hsspFilterGetCurveMinMaxIde

=item hsspFilterGetCurveMinMaxSim

=item hsspFilterGetCurveOld

=item hsspFilterGetCurveSim

=item hsspFilterGetPosExcl

=item hsspFilterGetPosIncl

=item hsspFilterGetRuleBoth

=item hsspFilterGetRuleSgi

=item hsspFilterMarkFile

=item hssp_filterSbr

=item hsspGetFile

=item hsspGetFileLoop
...
#    @PARAMS=split(/\s+/,&get_in_keyboard("command line"));	# x.x mac specific
				# ------------------------------
				# initialise variables
    ($Lok,$msg)=  
	&ini();                 if (! $Lok) { warn "*** ERROR $scrName after ini\n".$msg ;
					      Carp::confess( "*** during initialising $scrName" );}
...
#                            "------------------------------------------------------------\n";
    ...
    if ($#PARAMS > 1 && "$PARAMS[1] $PARAMS[2]" eq "help curve") {
	($Lok,$msg)=
	...
	next if ($arg=~/^(dirLib|ARCH|PWD|packName)=/);
        if    ($arg=~/^list$/i)             { $par{"isList"}=       1;}
	elsif ($arg=~/^de?bu?g$/i)          { $par{"debug"}=        1;} # dbg
            ...
            Carp::confess( "kwd '$arg' not understood: $0 @PARAMS" );
...
#	    if ($par{"verbose"});
	    ...
	    open($fhout_miss,">".$fileMissing) || Carp::confess( "failed to open $fileMissing: $!" ) ||
	...
	$par{"fileOutScreen"}=$par{"dirWork"}.$par{"fileOutScreen"}
		 ...
		 $par{"fileOutScreen"} ne "unk" && length($par{"fileOutScreen"}) > 0);
				...
				# ------------------------------
                                # trace file
    if( $par{"fileOutTrace"} && $par{"fileOutTrace"} ne "unk" && length($par{"fileOutTrace"}) > 0 && ! $par{"debug"} ){
...
#    $par{"fileMatGcg"}=         $par{"dirSrcMat"}.    "Maxhom_Blosum.metric";  # MAXHOM-GCG matrix
				...
				# --------------------
				# executables
#    $par{"exeConvertSeq"}=      $par{"dirBin"}.  "convert_seq";
    $par{"exeFilterHssp"}=      "filter_hssp"; # FORTRAN excutable
#    $par{"exeFilterHssp"}=      "/home/rost/lion/src-lib/filter_hssp"; # FORTRAN excutable
}				# end of iniDef
...
#                    if (! -x $par{"$kwd"});
	    ...
	    $msgHere.= "*** ERROR file ($kwd) '".$par{"$kwd"}."' missing! ".Carp::longmess
		if (! -e $par{"$kwd"} && ! -l $par{"$kwd"});} # 
    }
...
#       out:                    <0,$ARCH>
#-------------------------------------------------------------------------------
    # lkajan: the following seems obsolete
    return( 'LINUX' );
...
#-------------------------------------------------------------------------------
    ...
    if( $Lok ){ Carp::confess( "cmd $niceLoc \\cp $fileToCopy $fileToCopyTo failed: ".( $? >> 8 ) ); }
...
#    $LscreenTmp=1;
    ...
    return(0,"*** $sbrNamex: not def fhSbr!")                  if (! defined $fhSbr);
				...
				# ------------------------------
				# default settings
    $exeFilterHssp="filter_hssp" if ($exeFilterHssp eq "def");
    $fileMatGcg=   "$par{dirHome}/mat/Maxhom_GCG.metric"  if ($fileMatGcg eq "def");

    return(0,"*** $sbrNamex: miss in file '$fileInLoc'!")      if (! -e $fileInLoc);
    return(0,"*** $sbrNamex: miss in file '$fileMatGcg'!")     if (! -e $fileMatGcg);
    #lkajan: following does not take PATH into account
	...
	$ctIncl=1;}
				# ------------------------------
				# print onto screen
    if ($fhSbr && $par{debug}){print  $fhSbr "--- $sbrNamex: take no:";
    ...
    $anTable=      $fileTable   if ($fileTable);
				# ------------------------------
				# run filter_hssp
    $cmd="$exeFilterHssp,$fileTmp,$fileOutLoc,$fileMatGcg,$threshOld,$anRed,$anClean,$anTable ";
    ($Lok,$msg)=
